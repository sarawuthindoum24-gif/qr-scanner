...
function bot3ExecuteRoleJobV1_(job) {
  if (job.action !== 'ANALYZE_SYMBOL') throw new Error('Grace ไม่รองรับ action: ' + job.action);
  return bot3CompactTextV1_(buildStockifySymbolReportV748(job.symbol));
}
