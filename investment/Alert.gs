...
const BUY_ALERT_VERSION_V1000 = '10.3-buy-alert-openrouter-confirm-flow-smartalert';
...
function handleBuyAlertCommandV1000_(textRaw, replyToken) {
  ...
  const smartParsed = parseSmartAlertCommandV1110_(raw);
  if (smartParsed) {
    replyLine(replyToken, '⏳ รับคำสั่งแล้ว: Smart Alert ' + smartParsed.symbol + '\nระบบจะคำนวณแนวรับ/แนวต้าน + checklist + AI optional แล้วถามยืนยันก่อนบันทึก');
    enqueueTaskV748_({
      type: 'SMART_ALERT',
      symbol: smartParsed.symbol,
      amounts: smartParsed.amounts,
      repeatMinutes: smartParsed.repeatMinutes,
...
function parseSmartAlertCommandV1110_(raw) {
  const txt = String(raw || '').trim();
  const m = txt.match(/^(?:smartalert|smart|วางไม้|วางไม้ซื้อ|จัดไม้|ตั้งไม้)\s+(.+)$/i);
  if (!m) return null;

  const parts = m[1].split(/\s+/).filter(Boolean);
  if (!parts.length) throw new Error('ต้องระบุ symbol เช่น smartalert ASTS 500 700 800 r=30');
  ...
  parts.forEach(function(p) {
    ...
    const n = Number(String(p).replace(/,/g, ''));
    if (isFinite(n) && n > 0) amounts.push(n);
  });
...
function handlePendingBuyAlertReplyV1010_(raw, replyToken) {
  ...
  if (isCancel) {
    const cancelMsg = pending.action === 'CREATE_SMART_ALERT'
      ? 'ยกเลิกแล้วครับ ยังไม่สร้าง Smart Alert / ยังไม่บันทึกเข้า BuyAlerts'
      : 'ยกเลิกแล้วครับ ยังไม่อัปเดตสถานะซื้อใน Sheet';
    clearPendingBuyAlertActionV1010_();
    replyLine(replyToken, cancelMsg);
    return true;
  }
  ...
  if (isConfirm) {
    if (pending.action === 'CREATE_SMART_ALERT') {
      if (!pending.alertPayload) {
        clearPendingBuyAlertActionV1010_();
        replyLine(replyToken, '❌ Smart Alert หมดอายุหรือข้อมูลไม่ครบ กรุณาพิมพ์ smartalert อีกครั้ง');
        return true;
      }
      const saved = appendOrUpdateBuyAlertV1000_(pending.alertPayload);
      clearPendingBuyAlertActionV1010_();
      replyLine(replyToken, saved + '\n\n✅ บันทึก Smart Alert หลังยืนยันเรียบร้อย');
      return true;
    }
...
function createAutoBuyAlertFromAnalysisV1000_(symbol) {
  ...
  decision.news = cfg.currency === 'THB' ? emptyNewsV748_('Thai optional') : fetchFinnhubCompanyNewsV880_(cfg);
  if (cfg.currency === 'THB') decision.news.gatePass = true;
  ...
  return msg + '\n\n🧠 สถานะจากระบบ: ' + (decision.displayAction || decision.buyTier || 'WAIT') +
    ' | Score ' + decision.score + ' | Confidence ' + Math.round(decision.confidence || 0) + '%\n' +
    'เหตุผล: ' + (decision.reasons || []).slice(0, 3).join(' / ');
}

function createSmartAlertProposalV1110_(symbol, customAmounts, repeatMinutes) {
  symbol = sanitizeAlertSymbolV1000_(symbol);
  const cfg = getAlertSymbolConfigV1000_(symbol);
  const d = safeFetchTechnicalForAlertV1000_(cfg);
  if (!d || !d.ok) return '❌ Smart Alert ไม่สำเร็จ: ดึงข้อมูล ' + symbol + ' ไม่ได้';
  ...
  const entries = buildSmartAlertEntriesV1110_(cfg, d, decision, market, levels);
  const amounts = normalizeSmartAlertAmountsV1110_(cfg, customAmounts, decision);
  const priceObj = { ok: true, last: d.last, provider: d.provider || 'Technical', changePct: d.changePct };
  const ai = buildAlertAiDecisionV1000_(cfg, priceObj, null, []);
  const checklist = buildSmartAlertChecklistV1110_(cfg, d, decision, market, sr, ai);
  const resistanceGap = isFinite(sr.resistance) && d.last > 0 ? (sr.resistance - d.last) / d.last * 100 : null;
  const supportGap = isFinite(sr.support) && d.last > 0 ? (d.last - sr.support) / d.last * 100 : null;
  ...
  const alertPayload = {
    ...
    entries: entries,
    amounts: amounts,
    repeatMinutes: repeatMinutes || BUY_ALERT_DEFAULT_REPEAT_MIN_V1000,
    aiDecision: ai.decision || decision.buyTier || 'SMART_ALERT',
    aiScore: ai.score || '',
    aiNote: (ai.note || (decision.reasons || []).slice(0, 2).join(' / ')),
    note: 'Smart Alert: support/resistance + checklist + AI optional'
  };

  setPendingSmartAlertCreateV1110_(alertPayload, {
    support: sr.support,
    resistance: sr.resistance,
    supportGap: supportGap,
    resistanceGap: resistanceGap,
    decision: decision.displayAction || decision.buyTier || 'WAIT',
  ...
  return buildSmartAlertProposalMessageV1110_(cfg, d, decision, market, sr, entries, amounts, repeatMinutes, ai, checklist, supportGap, resistanceGap);
}

function buildSmartAlertEntriesV1110_(cfg, d, decision, market, levels) {
  levels = levels || buildSupportLevelsV773_(d);
  const last = Number(d && d.last);
  const entries = [levels.near, levels.mid, levels.deep]
    .map(function(x) { return roundMoneyV748_(Number(x), cfg.currency); })
    .filter(function(x) { return isFinite(x) && x > 0; });

  // ถ้าความเสี่ยงสูง ให้กดไม้แรกลงอีกนิดเพื่อไม่ให้ไล่ราคา
  if (decision && (decision.locked || decision.buyTier === 'WAIT')) {
    if (entries[0] && isFinite(last) && last > 0) entries[0] = roundMoneyV748_(Math.min(entries[0], last * 0.97), cfg.currency);
  }
  if (market && Number(market.heat || 0) >= MARKET_HEAT_HOT_V800) {
    if (entries[0] && isFinite(last) && last > 0) entries[0] = roundMoneyV748_(Math.min(entries[0], last * 0.965), cfg.currency);
    if (entries[1] && isFinite(last) && last > 0) entries[1] = roundMoneyV748_(Math.min(entries[1], last * 0.94), cfg.currency);
  }

  // บังคับเรียงจากสูงไปต่ำตาม format Buy Alert
  const clean = entries.slice(0, 3).filter(function(x) { return isFinite(x) && x > 0; });
  for (let i = 1; i < clean.length; i++) {
    if (clean[i] >= clean[i - 1]) clean[i] = roundMoneyV748_(clean[i - 1] * 0.995, cfg.currency);
  }
  return clean;
}

function normalizeSmartAlertAmountsV1110_(cfg, customAmounts, decision) {
  const raw = Array.isArray(customAmounts) ? customAmounts : [];
  const amounts = raw.slice(0, 3).map(function(x) { return Number(x); }).filter(function(x) { return isFinite(x) && x > 0; });
  while (amounts.length < 3) {
    if (decision && decision.plan && decision.plan.legs && decision.plan.legs[amounts.length]) {
      amounts.push(Number(decision.plan.legs[amounts.length].amount || 0));
    } else if ((cfg.currency || 'USD') === 'THB') {
      amounts.push([1000, 1000, 2000][amounts.length]);
    } else {
      amounts.push([5, 7, 8][amounts.length]);
    }
  }
  return amounts.slice(0, 3).map(function(x) { return roundMoneyV748_(x, cfg.currency); });
}

function buildSmartAlertChecklistV1110_(cfg, d, decision, market, sr, ai) {
  const out = [];
  const supportGap = isFinite(sr.support) && d.last > 0 ? (d.last - sr.support) / d.last * 100 : null;
  const resistanceGap = isFinite(sr.resistance) && d.last > 0 ? (sr.resistance - d.last) / d.last * 100 : null;

  out.push((d.last > d.ema20 ? '✅' : '⚠️') + ' ราคาเทียบ EMA20: ' + (d.last > d.ema20 ? 'เหนือ EMA20' : 'ยังต่ำกว่า EMA20'));
  out.push((d.ema20 > d.ema50 ? '✅' : '⚠️') + ' EMA20/50: ' + (d.ema20 > d.ema50 ? 'ขาขึ้น' : 'ยังไม่ยืนยัน'));
  out.push((d.macdHist >= 0 ? '✅' : '⚠️') + ' MACD: ' + (d.macdHist >= 0 ? 'บวก' : 'ยังลบ'));
  out.push((isRsiChecklistPassV820_(cfg, d.rsi) ? '✅' : '🔴') + ' RSI: ' + fmtNumV748_(d.rsi, 1) + ' | ' + rsiRegimeV820_(cfg, d.rsi).label);
  out.push((d.volatility <= VOL_HIGH_V800 ? '✅' : '🔴') + ' Volatility: ' + volTextV748_(d.volatility));
  out.push((Number(market.heat || 0) < MARKET_HEAT_HOT_V800 ? '✅' : '⚠️') + ' Market Heat: ' + Number(market.heat || 0) + '/100');
  out.push(((decision.news && decision.news.gatePass) || cfg.currency === 'THB' || cfg.cat === 'Core ETF' ? '✅' : '⚠️') + ' News Gate: ' + (((decision.news && decision.news.gatePass) || cfg.currency === 'THB' || cfg.cat === 'Core ETF') ? 'ผ่าน/ยกเว้น' : 'ยังไม่ผ่าน'));
  if (isFinite(supportGap)) out.push((supportGap <= 4 ? '✅' : '🟡') + ' ระยะถึงแนวรับ: ' + fmtPctV748_(supportGap));
  if (isFinite(resistanceGap)) out.push((resistanceGap >= 5 ? '✅' : '⚠️') + ' ระยะถึงแนวต้าน: ' + fmtPctV748_(resistanceGap));
  if (ai && ai.decision) out.push('🧠 AI: ' + ai.decision + (ai.score ? ' ' + ai.score + '/100' : ''));
  return out;
}

function setPendingSmartAlertCreateV1110_(alertPayload, meta) {
  setPendingBuyAlertActionV1010_({
    action: 'CREATE_SMART_ALERT',
    stage: 'AWAIT_CONFIRM',
...
function buildSmartAlertProposalMessageV1110_(cfg, d, decision, market, sr, entries, amounts, repeatMinutes, ai, checklist, supportGap, resistanceGap) {
  const cur = cfg.currency || 'USD';
  const lines = [];
  lines.push('🧠 Smart Alert Proposal: ' + cfg.symbol);
  lines.push('━━━━━━━━━━━━━━━━━━');
  lines.push('ราคาล่าสุด: ' + moneyV748_(d.last, cur) + ' | Data: ' + (d.provider || '-'));
  lines.push('แนวรับใช้งาน: ' + moneyV748_(sr.support, cur) + (isFinite(supportGap) ? ' (' + fmtPctV748_(supportGap) + ' จากราคา)' : ''));
  lines.push('แนวต้าน: ' + moneyV748_(sr.resistance, cur) + (isFinite(resistanceGap) ? ' (' + fmtPctV748_(resistanceGap) + ' จากราคา)' : ''));
  lines.push('สถานะระบบ: ' + (decision.displayAction || decision.buyTier || 'WAIT') + ' | Score ' + decision.score + ' | C' + Math.round(decision.confidence || 0) + '%');
  lines.push('Market Heat: ' + Number(market.heat || 0) + '/100 | ' + (market.regime || '-'));
  lines.push('');
  lines.push('📋 Checklist');
  (checklist || []).slice(0, 10).forEach(function(x) { lines.push('• ' + x); });
  lines.push('');
  lines.push('🎯 แผน Buy Alert 3 ไม้ที่บอทเสนอ');
  entries.slice(0, 3).forEach(function(entry, idx) {
    lines.push((idx + 1) + ') ' + moneyV748_(amounts[idx], cur) + ' @≤' + moneyV748_(entry, cur));
  });
  lines.push('เตือนซ้ำทุก: ' + (repeatMinutes || BUY_ALERT_DEFAULT_REPEAT_MIN_V1000) + ' นาที');
  lines.push('');
  lines.push('🧠 AI Decision: ' + (ai.decision || 'LOCAL') + (ai.score ? ' ' + ai.score + '/100' : ''));
  if (ai.action) lines.push('Action: ' + ai.action);
  if (ai.risk) lines.push('Risk: ' + ai.risk);
  if (ai.note) lines.push('เหตุผล: ' + ai.note);
  lines.push('');
  lines.push('ต้องการบันทึกแผนนี้เข้า BuyAlerts ไหม?');
  lines.push('ตอบ: ยืนยัน');
  lines.push('หรือ: ยกเลิก');
  lines.push('⚠️ บอทไม่ซื้อให้อัตโนมัติ เป็นเพียง alert + checklist ช่วยตัดสินใจ');
  return lines.join('\n');
}
...
function buildAiContextV1000_(cfg, price, alert, hitLegs, local) {
  const d = safeFetchTechnicalForAlertV1000_(cfg);
  const news = cfg.currency === 'THB' ? emptyNewsV748_('Thai optional') : fetchFinnhubCompanyNewsV880_(cfg);
  const ctx = {
    symbol: cfg.symbol,
    market: cfg.currency === 'THB' ? 'TH' : 'US',
    currency: cfg.currency || 'USD',
