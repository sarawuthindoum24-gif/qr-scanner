...
function buildNewsBuy3ReportV748() {
  // V11.0: Daily Yahoo + Investing News Bot.
  // Keeps old command name so Main.gs does not break.
  return buildDailyStockNewsReportV1100(false);
}
...
function fetchNewsForSymbolsV748_(configs) {
  // V11.0: Primary news sources requested by user = Yahoo + Investing RSS.
  // Finnhub remains available for Alert AI context only; daily news gate does not require it.
  return fetchStockNewsForSymbolsV1100_(configs, false);
}
...
function buildTerminalStatusMapV900_(d, decision) {
  return {
    ...
    news: decision.fastMode ? '⚪ FAST' : (decision.news && decision.news.gatePass ? '🟢 ✔️' : '🔴 ⛔')
  };
}
...
/* ============================================================
 * V11.0 DAILY YAHOO + INVESTING NEWS BOT
 * - Uses Watchlist from Google Sheets via getUSUniverseFromSheetV760_()
 * - Yahoo Finance search endpoint + Investing.com RSS feeds
 * - Mockup mode for testing before live schedule
 * - Optional OpenRouter summary if OPENROUTER_API_KEY exists
 *
 * Script Properties:
 *   LINE_TOKEN, LINE_USER_ID
 *   OPENROUTER_API_KEY          optional
 *   OPENROUTER_MODEL            optional, default openrouter/auto
 *   INVESTING_RSS_URLS          optional, newline/comma separated RSS urls
 * ============================================================ */

const DAILY_NEWS_VERSION_V1100 = '11.0-yahoo-investing-daily-news';
const DAILY_NEWS_TRIGGER_HANDLER_V1100 = 'sendDailyStockNewsV1100';

function setupDailyNewsTriggerV1100(hour, minute) {
  hour = isFinite(Number(hour)) ? Number(hour) : 20;
  minute = isFinite(Number(minute)) ? Number(minute) : 30;
  hour = Math.max(0, Math.min(23, hour));
  minute = Math.max(0, Math.min(59, minute));

  clearDailyNewsTriggerV1100_();
  ScriptApp.newTrigger(DAILY_NEWS_TRIGGER_HANDLER_V1100)
    .timeBased()
    .everyDays(1)
    .atHour(hour)
    .nearMinute(minute)
  ...
  PropertiesService.getScriptProperties().setProperty('DAILY_NEWS_HOUR', String(hour));
  PropertiesService.getScriptProperties().setProperty('DAILY_NEWS_MINUTE', String(minute));

  return [
    '✅ ตั้งส่งข่าวหุ้นรายวันสำเร็จ',
    'เวลาโดยประมาณ: ' + pad2V1100_(hour) + ':' + pad2V1100_(minute) + ' น. (' + TZ + ')',
    'แหล่งข่าว: Yahoo Finance + Investing.com RSS',
    '',
    'คำสั่งทดลอง:',
    '• mocknews = ดู Mockup',
    '• ข่าว = ดึงข่าวจริงตอนนี้',
    '• health = ตรวจระบบ'
  ].join('\n');
}

function clearDailyNewsTriggerV1100_() {
  const triggers = ScriptApp.getProjectTriggers();
  triggers.forEach(function(t) {
    if (t.getHandlerFunction && t.getHandlerFunction() === DAILY_NEWS_TRIGGER_HANDLER_V1100) {
      try { ScriptApp.deleteTrigger(t); } catch (e) {}
    }
  });
}

function deleteDailyNewsTriggerV1100() {
  clearDailyNewsTriggerV1100_();
  return '✅ ปิด Daily News Trigger แล้ว';
}

function sendDailyStockNewsV1100() {
  Logger.log('V13: daily news disabled; Khao is the news bot');
  return [];
}

function testDailyNewsMockupV1100() {
  sendManyLine(buildDailyStockNewsReportV1100(true));
}

function testDailyNewsLiveV1100() {
  sendManyLine(buildDailyStockNewsReportV1100(false));
}

function previewDailyNewsMockupV1100() {
  return buildDailyStockNewsReportV1100(true).join('\n\n---\n\n');
}

function buildDailyStockNewsReportV1100(useMock) {
  if (useMock) return buildMockDailyStockNewsReportV1100_();

  const now = Utilities.formatDate(new Date(), TZ, 'yyyy-MM-dd HH:mm');
  const all = getUSUniverseFromSheetV760_();
  const universe = all.filter(function(cfg) { return cfg && cfg.currency !== 'THB'; }).slice(0, 40);
  if (!universe.length) {
    return [
      [
        '⚠️ Daily Stock News v' + DAILY_NEWS_VERSION_V1100,
        'ยังไม่พบ Watchlist US ในระบบ',
        'ให้รัน setupWatchlistSheetV760() ก่อน หรือพิมพ์ +NVDA เพื่อเพิ่มหุ้น'
      ].join('\n')
    ];
  }

  const data = fetchManyTechnicalDataV748_(universe);
  const news = fetchStockNewsForSymbolsV1100_(universe, false);
  const market = buildMarketSnapshotV748_(data);

  const scored = universe.map(function(cfg) {
    const d = data[cfg.symbol];
    const decision = buildSymbolDecisionV748_(cfg, d, market, true);
    decision.news = news[cfg.symbol] || emptyNewsV748_();
    applyBuyTierLogicV753_(decision, d, market);
    decision.newsImpact = buildNewsImpactV1100_(decision.news);
    return decision;
  });

  const withNews = scored.filter(function(x) { return x.news && x.news.gatePass; })
    .sort(function(a, b) {
      return Math.abs(b.newsImpact.score) - Math.abs(a.newsImpact.score) ||
             Math.abs(b.changePct || 0) - Math.abs(a.changePct || 0);
    })
    .slice(0, 12);

  const noNews = scored.filter(function(x) { return !(x.news && x.news.gatePass); }).slice(0, 8);

  const yCount = scored.reduce(function(sum, x) { return sum + Number(x.news && x.news.yahoo || 0); }, 0);
  const iCount = scored.reduce(function(sum, x) { return sum + Number(x.news && x.news.investing || 0); }, 0);

  const header = [
    '🇺🇸 Daily Stock News v' + DAILY_NEWS_VERSION_V1100,
    'เวลา: ' + now + ' น.',
    'แหล่งข่าว: Yahoo Finance + Investing.com RSS',
    'Watchlist: ' + universe.length + ' ตัว | News Gate: Y' + yCount + '/I' + iCount,
    'Market Heat: ' + market.heat + '/100 ' + heatEmojiV748_(market.heat) + ' | Regime: ' + market.regime,
    'VIX: ' + fmtNumV748_(market.vix, 2) + ' | USD/THB: ฿' + fmtNumV748_(market.usdthb, 2),
    'หมายเหตุ: ถ้า Y0/I0 ระบบจะไม่มโนข่าว และให้รอข้อมูลยืนยัน'
  ].join('\n');

  const lines = [header, '', '🗞️ ข่าวสำคัญใน Watchlist'];
  if (!withNews.length) {
    lines.push('วันนี้ยังไม่พบข่าวตรงกับ Watchlist จาก Yahoo/Investing ในรอบนี้');
  } else {
    withNews.forEach(function(x, idx) {
      appendDailyNewsItemV1100_(lines, x, idx + 1);
    });
  }

  if (noNews.length) {
    lines.push('');
    lines.push('👀 ไม่มีข่าวตรงในรอบนี้');
    lines.push(noNews.map(function(x) { return x.symbol; }).join(', '));
  }

  lines.push('');
  lines.push('🎯 Action Guard');
  lines.push('• บอทสรุปข่าวและโซนเท่านั้น ไม่ส่งคำสั่งซื้ออัตโนมัติ');
  lines.push('• ถ้าข่าวขัดแย้ง/ข้อมูลไม่พอ ให้ถามกลับก่อนตัดสินใจทุกครั้ง');
  lines.push('• ก่อนซื้อจริงให้เช็กราคาล่าสุดในโบรกเกอร์อีกครั้ง');

  const aiText = summarizeDailyNewsWithAiV1100_(scored, market);
  const report1 = lines.join('\n');

  const buyList = scored.filter(function(x) {
      return (x.buyTier === 'FULL_BUY' || x.buyTier === 'TECH_BUY') && x.plan && x.plan.total > 0;
    })
    .sort(function(a, b) {
      return (a.buyTier === b.buyTier ? 0 : a.buyTier === 'FULL_BUY' ? -1 : 1) || b.score - a.score;
    })
    .slice(0, 8);

  const output = [report1];
  if (aiText) output.push(aiText);
  output.push(buildBuyOnlyMessageV748_(buyList, '🎯 แผนเข้าซื้อ 3 ไม้เท่านั้น'));
  return output;
}

function appendDailyNewsItemV1100_(lines, x, idx) {
  const n = x.news || emptyNewsV748_();
  const impact = x.newsImpact || buildNewsImpactV1100_(n);
  const icon = impact.score > 0 ? '🟢' : impact.score < 0 ? '🔴' : '⚪';
  const source = buildNewsSourceTextV1100_(n);
  const headline = n.headline || 'ไม่มีหัวข่าว';
  const change = isFinite(x.changePct) ? fmtPctV748_(x.changePct) : '-';
  const action = buildNewsActionGuardV1100_(x, impact);

  lines.push('');
  lines.push(idx + '. ' + icon + ' ' + x.symbol + ' ' + change + ' | Impact ' + impact.label + ' | ' + source);
  lines.push('   ข่าว: ' + truncateV748_(headline, 145));
  lines.push('   มุมมอง: ' + impact.reason);
  lines.push('   Guard: ' + action);
}

function buildNewsSourceTextV1100_(n) {
  const parts = [];
  if (Number(n.yahoo || 0) > 0) parts.push('Yahoo ' + Number(n.yahoo || 0));
  if (Number(n.investing || 0) > 0) parts.push('Investing ' + Number(n.investing || 0));
  if (Number(n.finnhub || 0) > 0) parts.push('Finnhub ' + Number(n.finnhub || 0));
  return parts.length ? parts.join(' / ') : 'No source';
}

function buildNewsImpactV1100_(n) {
  n = n || emptyNewsV748_();
  const score = Math.max(-3, Math.min(3, Number(n.score || 0)));
  let label = 'Neutral';
  if (score >= 2) label = 'Bullish+';
  else if (score > 0) label = 'Bullish';
  else if (score <= -2) label = 'Bearish-';
  else if (score < 0) label = 'Bearish';

  const headline = String(n.headline || '');
  let reason = 'ข่าวมีผลกลาง ๆ ต้องดูราคายืนยัน';
  if (!n.gatePass) reason = 'ยังไม่มีข่าวตรงจาก Yahoo/Investing ห้ามสรุปเอง';
  else if (/downgrade|probe|lawsuit|miss|weak|cut|falls|sinks|plunge|tumble|delay/i.test(headline)) reason = 'มีคำเชิงลบ/ความเสี่ยงในหัวข่าว';
  else if (/upgrade|raises|beat|strong|growth|record|partnership|contract|wins|outperform/i.test(headline)) reason = 'มีคำเชิงบวก/แรงหนุนในหัวข่าว';
  else if (score > 0) reason = 'sentiment ข่าวเป็นบวกเล็กน้อย';
  else if (score < 0) reason = 'sentiment ข่าวเป็นลบเล็กน้อย';

  return { score: score, label: label, reason: reason };
}

function buildNewsActionGuardV1100_(decision, impact) {
  if (!decision || !decision.news || !decision.news.gatePass) return 'รอข่าวยืนยันก่อน ไม่ตัดสินใจจากข้อมูลว่าง';
  if (impact && impact.score < 0) return 'ระวังข่าวลบ รอดูราคา/วอลุ่มก่อน';
  if (decision.buyTier === 'FULL_BUY') return 'ผ่านเทคนิค แต่ให้ถามยืนยันก่อนซื้อจริง';
  if (decision.buyTier === 'TECH_BUY') return 'เข้าได้เฉพาะไม้เล็ก ถ้าราคายืนยัน';
  return 'ติดตามต่อ ยังไม่ต้องรีบไล่ราคา';
}

function fetchStockNewsForSymbolsV1100_(configs, useMock) {
  if (useMock) return buildMockNewsMapV1100_(configs || []);

  configs = (configs || []).filter(function(cfg) { return cfg && cfg.symbol; });
  const yahoo = fetchYahooNewsForSymbolsV880_(configs);
  const investing = fetchInvestingNewsForSymbolsV1100_(configs);
  const out = {};

  configs.forEach(function(cfg) {
    out[cfg.symbol] = mergeYahooInvestingNewsV1100_(
      cfg,
      yahoo[cfg.symbol] || emptyNewsV748_('Yahoo no data'),
      investing[cfg.symbol] || emptyNewsV748_('Investing no data')
    );
  });
...
function mergeYahooInvestingNewsV1100_(cfg, yahoo, investing) {
  yahoo = yahoo || emptyNewsV748_();
  investing = investing || emptyNewsV748_();

  const yCount = Number(yahoo.yahoo || 0);
  const iCount = Number(investing.investing || 0);
  const combinedScore = Math.max(-3, Math.min(3, Number(yahoo.score || 0) + Number(investing.score || 0)));
  const headline = yahoo.headline || investing.headline || '';
  const pass = yCount > 0 || iCount > 0 || cfg.cat === 'Core ETF';

  const headlines = [];
  if (yahoo.headline) headlines.push({ source: 'Yahoo', title: yahoo.headline, link: yahoo.link || '' });
  (investing.items || []).slice(0, 3).forEach(function(it) {
    headlines.push({ source: 'Investing', title: it.title, link: it.link || '' });
  });
  ...
  return {
    ...
    investing: iCount,
    thai: 0,
    http: yahoo.http || investing.http || 0,
    score: combinedScore,
    headline: headline,
    gatePass: pass,
    status: pass ? 'PASS' : 'BLOCK',
    source: buildMergedSourceLabelV1100_(yCount, iCount),
    items: headlines,
    error: pass ? '' : ((yahoo.error || '') + ' ' + (investing.error || '')).trim()
  };
}

function buildMergedSourceLabelV1100_(yCount, iCount) {
  const parts = [];
  if (yCount > 0) parts.push('Yahoo');
  if (iCount > 0) parts.push('Investing');
  return parts.join('+');
}

function fetchInvestingNewsForSymbolsV1100_(configs) {
  const items = fetchInvestingRssItemsV1100_();
  const out = {};
  configs.forEach(function(cfg) {
    const aliases = buildSymbolAliasesV1100_(cfg);
    const relevant = items.filter(function(item) {
      return isRelevantInvestingItemV1100_(item, aliases, cfg);
    }).slice(0, 6);

    const score = relevant.reduce(function(sum, item) {
      return sum + sentimentScoreV748_(item.title || '');
    }, 0);

    out[cfg.symbol] = {
      yahoo: 0,
      finnhub: 0,
      investing: relevant.length,
      thai: 0,
      http: items.length ? 200 : 0,
      score: Math.max(-3, Math.min(3, score)),
      headline: relevant.length ? relevant[0].title : '',
      gatePass: relevant.length > 0,
      status: relevant.length ? 'PASS' : 'BLOCK',
      source: relevant.length ? 'Investing' : '',
      items: relevant,
      error: relevant.length ? '' : 'No relevant Investing RSS item'
    };
  });
  return out;
}

function fetchInvestingRssItemsV1100_() {
  const urls = getInvestingRssUrlsV1100_();
  const reqs = urls.map(function(url) {
    return {
      ...
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; StockifyBot/11.0; +https://script.google.com)',
  ...
  try {
    ...
    resps.forEach(function(res, idx) {
      ...
      if (code >= 200 && code < 300 && body) {
        all = all.concat(parseRssItemsV1100_(body, urls[idx]));
      }
    });
  } catch (err) {
    Logger.log('fetchInvestingRssItemsV1100_ error: ' + (err.stack || err));
  }

  return uniqueNewsItemsV1100_(all).slice(0, 120);
}

function getInvestingRssUrlsV1100_() {
  const prop = PropertiesService.getScriptProperties().getProperty('INVESTING_RSS_URLS');
  if (prop) {
    const custom = prop.split(/[\n,]+/).map(function(x) { return String(x || '').trim(); }).filter(Boolean);
    if (custom.length) return custom;
  }

  return [
    'https://th.investing.com/rss/news.rss',
    'https://th.investing.com/rss/news_25.rss',
    'https://th.investing.com/rss/news_285.rss',
    'https://th.investing.com/rss/market_overview.rss',
    'https://th.investing.com/rss/stock_Stocks.rss'
  ];
}

function parseRssItemsV1100_(xmlText, feedUrl) {
  const out = [];
  try {
    ...
    items.forEach(function(item) {
      const title = getChildTextV1100_(item, 'title');
      const link = getChildTextV1100_(item, 'link');
      const pubDate = getChildTextV1100_(item, 'pubDate');
      const description = getChildTextV1100_(item, 'description');
      if (!title) return;
      out.push({
        title: cleanTextV1100_(title),
        link: String(link || '').trim(),
        pubDate: String(pubDate || '').trim(),
        timestamp: pubDate ? Date.parse(pubDate) : 0,
        summary: cleanTextV1100_(description),
        source: 'Investing',
        feed: feedUrl
      });
    });
  } catch (err) {
    Logger.log('parseRssItemsV1100_ error: ' + err.message + ' | feed=' + feedUrl);
  }
  return out;
}

function getChildTextV1100_(el, name) {
  const child = el.getChild(name);
  return child ? child.getText() : '';
}

function cleanTextV1100_(s) {
  s = String(s || '');
  s = s.replace(/<[^>]+>/g, ' ');
  s = s.replace(/&nbsp;/g, ' ')
       .replace(/&amp;/g, '&')
...
function uniqueNewsItemsV1100_(items) {
  const seen = {};
  const out = [];
...
function buildSymbolAliasesV1100_(cfg) {
  const aliases = (SYMBOL_ALIASES_V748[cfg.symbol] || [cfg.symbol]).slice();
  aliases.push(cfg.symbol);
  if (cfg.yahoo) aliases.push(String(cfg.yahoo).replace('.BK', ''));
  const seen = {};
  return aliases.map(function(a) { return String(a || '').trim(); })
    .filter(function(a) {
...
function isRelevantInvestingItemV1100_(item, aliases, cfg) {
  const text = (String(item.title || '') + ' ' + String(item.summary || '')).toLowerCase();

  if (cfg.cat === 'Core ETF' || ['VOO','QQQM','SMH','SCHG','QTUM'].indexOf(cfg.symbol) >= 0) {
    const macro = /s&p|nasdaq|dow|stock market|wall street|fed|rate|inflation|treasury|yield|semiconductor|chip|ai|etf/i.test(text);
    const aliasHit = aliases.some(function(a) { return text.indexOf(String(a).toLowerCase()) >= 0; });
    return macro || aliasHit;
  }
  ...
  return aliases.some(function(a) {
    ...
    if (lower.length <= 3) {
      const re = new RegExp('(^|[^a-z0-9])' + escapeRegexV1100_(lower) + '([^a-z0-9]|$)', 'i');
      return re.test(text);
    }
...
function escapeRegexV1100_(s) {
  return String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

function summarizeDailyNewsWithAiV1100_(scored, market) {
  try {
    const props = PropertiesService.getScriptProperties();
    const apiKey = props.getProperty('OPENROUTER_API_KEY');
    if (!apiKey) return '';

    const top = (scored || []).filter(function(x) { return x.news && x.news.gatePass; }).slice(0, 12).map(function(x) {
      return {
        symbol: x.symbol,
        changePct: x.changePct,
        buyTier: x.buyTier,
        score: x.score,
        newsScore: x.news.score,
        source: x.news.source,
        headline: x.news.headline
      };
    });
    ...
    const payload = {
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a disciplined Thai stock-market news summarizer. Use only provided data. Never invent news. Do not guarantee profit. Keep concise for LINE.'
        },
        {
          role: 'user',
          content: JSON.stringify({
            market: {
              heat: market && market.heat,
              regime: market && market.regime,
              vix: market && market.vix,
              usdthb: market && market.usdthb
            },
            watchlist_news: top
          })
        }
    ...
    const res = UrlFetchApp.fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Stockify Daily News Bot'
      },
      payload: JSON.stringify(payload),
      muteHttpExceptions: true
    });
    ...
    if (res.getResponseCode() < 200 || res.getResponseCode() >= 300) {
      Logger.log('OpenRouter Daily News error ' + res.getResponseCode() + ': ' + res.getContentText());
      return '';
    }
    ...
    if (!text) return '';
    return '🧠 AI Summary\n' + truncateV748_(String(text).trim(), 1800);
  } catch (err) {
    Logger.log('summarizeDailyNewsWithAiV1100_ error: ' + (err.stack || err));
    return '';
  }
}

function buildMockDailyStockNewsReportV1100_() {
  const now = Utilities.formatDate(new Date(), TZ, 'yyyy-MM-dd HH:mm');
  const lines = [
    '🧪 MOCKUP Daily Stock News v' + DAILY_NEWS_VERSION_V1100,
    'เวลา: ' + now + ' น.',
    'แหล่งข่าวจำลอง: Yahoo Finance + Investing.com RSS',
    'Watchlist: 8 ตัว | News Gate: Y4/I5',
    'Market Heat: 54/100 🟡 กลาง | Regime: Neutral',
    'VIX: 16.20 | USD/THB: ฿36.20',
    'หมายเหตุ: ข้อความนี้เป็น mockup เพื่อเช็กรูปแบบ LINE ก่อนดึงข่าวจริง',
    '',
    '🗞️ ข่าวสำคัญใน Watchlist',
    '',
    '1. 🟢 NVDA +2.10% | Impact Bullish+ | Yahoo 1 / Investing 1',
    '   ข่าว: Nvidia supplier optimism lifts AI chip names after analysts raise data-center estimates',
    '   มุมมอง: มีคำเชิงบวก/แรงหนุนในหัวข่าว',
    '   Guard: ผ่านเทคนิค แต่ให้ถามยืนยันก่อนซื้อจริง',
    '',
    '2. ⚪ AVGO -0.80% | Impact Neutral | Yahoo 1',
    '   ข่าว: Broadcom investors await AI revenue update as semiconductor sector consolidates',
    '   มุมมอง: ข่าวมีผลกลาง ๆ ต้องดูราคายืนยัน',
    '   Guard: ติดตามต่อ ยังไม่ต้องรีบไล่ราคา',
    '',
    '3. 🔴 RGTI -5.40% | Impact Bearish- | Investing 1',
    '   ข่าว: Quantum computing stocks pull back as risk appetite fades',
    '   มุมมอง: มีคำเชิงลบ/ความเสี่ยงในหัวข่าว',
    '   Guard: ระวังข่าวลบ รอดูราคา/วอลุ่มก่อน',
    '',
    '🎯 Action Guard',
    '• บอทสรุปข่าวและโซนเท่านั้น ไม่ส่งคำสั่งซื้ออัตโนมัติ',
    '• ถ้าข่าวขัดแย้ง/ข้อมูลไม่พอ ให้ถามกลับก่อนตัดสินใจทุกครั้ง',
    '• ก่อนซื้อจริงให้เช็กราคาล่าสุดในโบรกเกอร์อีกครั้ง'
  ];

  const plan = [
    '🎯 แผนเข้าซื้อ 3 ไม้เท่านั้น',
    'ตัวอย่าง mockup:',
    '1. NVDA TECH BUY | ข่าวผ่าน แต่ราคาเริ่มร้อน',
    '   ไม้ 1: $8.00 @≤$900.00',
    '   ไม้ 2: $12.00 @≤$860.00',
    '   ไม้ 3: $20.00 @≤$820.00',
    '⚠️ MOCKUP ไม่ใช่ข้อมูลจริง'
  ];

  return [lines.join('\n'), plan.join('\n')];
}

function buildMockNewsMapV1100_(configs) {
  const out = {};
  (configs || []).forEach(function(cfg, idx) {
    const title = idx % 3 === 0
      ? cfg.symbol + ' analysts raise target on AI growth'
      : idx % 3 === 1
        ? cfg.symbol + ' stock slips as investors wait for earnings'
        : cfg.symbol + ' market update remains neutral';
    out[cfg.symbol] = {
      yahoo: idx % 2 === 0 ? 1 : 0,
      finnhub: 0,
      investing: idx % 2 === 1 ? 1 : 0,
      thai: 0,
      score: sentimentScoreV748_(title),
      headline: title,
      gatePass: true,
      status: 'PASS',
      source: idx % 2 === 0 ? 'Yahoo' : 'Investing',
      items: [{ source: idx % 2 === 0 ? 'Yahoo' : 'Investing', title: title, link: '' }]
    };
  });
  return out;
}

function pad2V1100_(n) {
  n = Number(n);
  return (n < 10 ? '0' : '') + n;
}


function setupNewsTriggers2SlotsV1100() {
  // Remove existing news triggers for sendDailyStockNewsV1100
  ScriptApp.getProjectTriggers().forEach(function(t){
    if (t.getHandlerFunction() === 'sendDailyStockNewsV1100') ScriptApp.deleteTrigger(t);
  });
  // Create two daily triggers: 10:00 and 19:30
  ScriptApp.newTrigger('sendDailyStockNewsV1100').timeBased().atHour(10).nearMinute(0).everyDays(1).create();
  ScriptApp.newTrigger('sendDailyStockNewsV1100').timeBased().atHour(19).nearMinute(30).everyDays(1).create();
  Logger.log('Created 2 news triggers: 10:00 and 19:30');
}


// ==================== รวมสรุปข่าวทั้งหมด (Dashboard + ข่าวหุ้น) 2 ช่วง 10:00 / 19:30 ====================
function sendCombinedNewsSummaryV1200() {
  Logger.log('V13: combined news disabled; Khao is the news bot');
  return { ok: true, disabled: true, owner: 'Khao' };
}

function setupCombinedNewsTriggers2SlotsV1200() {
  // ลบ trigger เดิมของข่าว/dashboard ทั้งหมด กันซ้ำซ้อน
  var oldHandlers = ['sendDailyStockNewsV1100', 'ecMarketDashboard', 'sendCombinedNewsSummaryV1200'];
  ScriptApp.getProjectTriggers().forEach(function (t) {
    if (oldHandlers.indexOf(t.getHandlerFunction()) !== -1) {
      try { ScriptApp.deleteTrigger(t); } catch (e) {}
    }
  });
  // สร้าง trigger รวม 2 ช่วง: 10:00 และ 19:30 (ทุกวัน)
  ScriptApp.newTrigger('sendCombinedNewsSummaryV1200').timeBased().atHour(10).nearMinute(0).everyDays(1).create();
  ScriptApp.newTrigger('sendCombinedNewsSummaryV1200').timeBased().atHour(19).nearMinute(30).everyDays(1).create();
  Logger.log('ติดตั้ง trigger รวมสรุปข่าวทั้งหมด 2 ช่วง: 10:00 และ 19:30 เรียบร้อย');
  return '✅ ตั้ง trigger รวมสรุปข่าวทั้งหมด (dashboard + ข่าวหุ้น) 2 ช่วง 10:00 และ 19:30 เรียบร้อย';
}

// ทดสอบส่งสรุปข่าวทั้งหมดทันที (ไม่ต้องรอ trigger)
function testCombinedNewsSummaryV1200() {
  return sendCombinedNewsSummaryV1200();
}




// ==================== Webull OpenAPI (Market Data / Snapshot) V1200 ====================
// ต้องตั้งค่าใน Script Properties: WEBULL_APP_KEY, WEBULL_APP_SECRET, WEBULL_HOST
//   Production host: api.webull.co.th | Test host: th-api.uat.webullbroker.com
//   (Test env ใช้ public test credentials ได้ ไม่ต้องทำ 2FA)
// อย่าฝัง App Secret ในโค้ด ให้ใส่ใน Script Properties เท่านั้น
var WEBULL_TEST_HOST_V1200 = 'th-api.uat.webullbroker.com';

function webullGetConfigV1200_() {
  var p = PropertiesService.getScriptProperties();
  var host = p.getProperty('WEBULL_HOST') || WEBULL_TEST_HOST_V1200;
  var appKey = p.getProperty('WEBULL_APP_KEY') || '';
  var appSecret = p.getProperty('WEBULL_APP_SECRET') || '';
  var token = p.getProperty('WEBULL_ACCESS_TOKEN') || '';
  return { host: host, appKey: appKey, appSecret: appSecret, token: token };
}

function webullSha256HexUpperV1200_(str) {
...
// สร้าง signature ตาม algorithm ของ Webull (SHA256 body hash + HMAC-SHA256, key = secret + '&')
function webullSignV1200_(path, queryParams, bodyString, appKey, appSecret, host, timestamp, nonce) {
  var signHeaders = {
    'x-app-key': appKey,
  ...
  var all = {};
  var k;
  if (queryParams) { for (k in queryParams) all[k] = String(queryParams[k]); }
  for (k in signHeaders) all[k] = signHeaders[k];
  var keys = Object.keys(all).sort();
  var pairs = [];
  for (var i = 0; i < keys.length; i++) pairs.push(keys[i] + '=' + all[keys[i]]);
  var str1 = pairs.join('&');
  var str3 = path + '&' + str1;
  if (bodyString) { str3 = str3 + '&' + webullSha256HexUpperV1200_(bodyString); }
  var encoded = encodeURIComponent(str3);
  var raw = Utilities.computeHmacSha256Signature(
    Utilities.newBlob(encoded).getBytes(),
    Utilities.newBlob(appSecret + '&').getBytes()
  );
  return Utilities.base64Encode(raw);
}

// เรียก Webull API (GET/POST) แบบ signed. คืน { code, json, text }
function webullCallV1200_(method, path, queryParams, body) {
  var cfg = webullGetConfigV1200_();
  if (!cfg.appKey || !cfg.appSecret) {
    return { code: 0, error: 'ยังไม่ได้ตั้ง WEBULL_APP_KEY / WEBULL_APP_SECRET ใน Script Properties' };
  }
  queryParams = queryParams || {};
  var timestamp = Utilities.formatDate(new Date(), 'UTC', "yyyy-MM-dd'T'HH:mm:ss'Z'");
  var nonce = Utilities.getUuid().replace(/-/g, '');
  var bodyString = body ? JSON.stringify(body) : null;
  var signature = webullSignV1200_(path, queryParams, bodyString, cfg.appKey, cfg.appSecret, cfg.host, timestamp, nonce);
  var headers = {
  ...
  var qs = '';
  var qk = Object.keys(queryParams);
  if (qk.length) { qs = '?' + qk.map(function (k) { return k + '=' + encodeURIComponent(queryParams[k]); }).join('&'); }
  var options = { method: method, headers: headers, muteHttpExceptions: true };
  if (bodyString) { options.contentType = 'application/json'; options.payload = bodyString; }
  var res = UrlFetchApp.fetch('https://' + cfg.host + path + qs, options);
...
// ดึง snapshot ราคาหุ้น/ETF สหรัฐฯ (รองรับหลาย symbol คั่นด้วย comma)
function webullSnapshotV1200(symbols, category) {
  category = category || 'US_STOCK';
  return webullCallV1200_('GET', '/openapi/market-data/stock/snapshot', {
    symbols: symbols,
    category: category,
...
// ทดสอบ: ยืนยันว่า signature + การเรียก API ใช้งานได้ (ใช้ test env ได้เลย)
function testWebullSnapshotV1200() {
  var r = webullSnapshotV1200('AAPL', 'US_STOCK');
  Logger.log('HTTP ' + r.code);
  Logger.log(r.text ? r.text.slice(0, 500) : r.error);
  if (r.json && r.json[0]) {
    var s = r.json[0];
    Logger.log('OK: ' + s.symbol + ' price=' + s.price + ' change_ratio=' + s.change_ratio);
  }
  return r;
}


// ===== Webull US price summary (V1200) =====
function webullBarsV1200_(symbol, category, timespan, count) {
  category = category || 'US_STOCK';
  timespan = timespan || 'D';
  count = count || '60';
  return webullCallV1200_('GET', '/openapi/market-data/stock/bars', {
    symbol: symbol,
    category: category,
    timespan: timespan,
    count: count,
...
// Compute RSI(14) from Webull daily bars. Returns a number or null on failure.
function webullRsiV1200_(symbol, category) {
  try {
    var res = webullBarsV1200_(symbol, category, 'D', '60');
    if (!res || res.code !== 200 || !res.json || !res.json.length) return null;
    var bars = res.json.slice();
    // Webull returns latest first; sort ascending by time so RSI uses chronological closes
    bars.sort(function (a, b) { return String(a.time) < String(b.time) ? -1 : 1; });
    var closes = [];
    for (var i = 0; i < bars.length; i++) {
      var c = parseFloat(bars[i].close);
      if (!isNaN(c)) closes.push(c);
    }
    if (closes.length < 15) return null;
    return rsiV748_(closes, 14);
  } catch (e) {
    Logger.log('webullRsiV1200_ ' + symbol + ': ' + e);
    return null;
  }
}

// Technical RSI status (classic 30/70). Informational only, not investment advice.
function webullRsiStatusV1200_(rsi) {
  if (rsi === null || isNaN(rsi)) return '';
  var r = Math.round(rsi);
  if (rsi < 30) return '🟢 RSI ' + r + ' – Oversold (ขายมากเกิน)';
  if (rsi > 70) return '🔴 RSI ' + r + ' – Overbought (ซื้อมากเกิน)';
  return '⚪ RSI ' + r + ' – Neutral';
}

function buildWebullUsPricesV1200_() {
  var props = PropertiesService.getScriptProperties();
  var raw = props.getProperty('WEBULL_US_SYMBOLS');
  var symbols = (raw && raw.trim()) ? raw.trim() : 'AAPL,NVDA,MSFT,GOOGL,AMZN,META,TSLA,VOO';
  var res = webullSnapshotV1200(symbols, 'US_STOCK');
  if (!res || res.code !== 200 || !res.json || !res.json.length) {
    throw new Error('Webull snapshot code=' + (res ? res.code : 'null'));
  }
  var lines = ['🇺🇸 ราคาหุ้น US (Webull)'];
  res.json.forEach(function (s) {
    var cr = parseFloat(s.change_ratio);
    var pct = isNaN(cr) ? '-' : (cr * 100).toFixed(2) + '%';
    var arrow = isNaN(cr) ? '•' : (cr > 0 ? '🔺' : (cr < 0 ? '🔻' : '▫'));
    var sign = (!isNaN(cr) && cr > 0) ? '+' : '';
    var rsi = webullRsiV1200_(s.symbol, 'US_STOCK');
    var rsiStr = webullRsiStatusV1200_(rsi);
    var line = arrow + ' ' + s.symbol + ' ' + s.price + ' (' + sign + pct + ')';
    if (rsiStr) line += '\n    ' + rsiStr;
    lines.push(line);
  });
  lines.push('ℹ️ ข้อมูลเชิงเทคนิค ไม่ใช่คำแนะนำการลงทุน');
  return lines.join('\n');
}

function testWebullUsPricesV1200() {
  var msg = buildWebullUsPricesV1200_();
  Logger.log(msg);
  return msg;
}
